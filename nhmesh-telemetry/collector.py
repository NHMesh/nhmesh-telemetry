...
def parse_standard_meshtastic_packet(raw_packet_dict):
    ...
    try:
        ...
        lat = float(position_part.get("latitudeI")) / 1e7 if position_part.get("latitudeI") is not None else None
        lon = float(position_part.get("longitudeI")) / 1e7 if position_part.get("longitudeI") is not None else None

        parsed_data = {
            "from_id_num": raw_packet_dict.get("from"), 
            "to_id_num": raw_packet_dict.get("to"),
            "portnum": decoded_part.get("portnum"), 
            "payload_raw": payload_raw_str,
            "decoded_content": decoded_part, 
            "telemetry_time": telemetry_part.get("time"),
            "battery_level": device_metrics.get("batteryLevel") or raw_packet_dict.get("payload", {}).get("battery_level"),
            "voltage": device_metrics.get("voltage") or raw_packet_dict.get("payload", {}).get("voltage"),
            "channel_utilization": device_metrics.get("channelUtilization") or raw_packet_dict.get("payload", {}).get("channel_utilization"),
            "air_util_tx": device_metrics.get("airUtilTx") or raw_packet_dict.get("payload", {}).get("air_util_tx"),
            "uptime_seconds": device_metrics.get("uptimeSeconds") or raw_packet_dict.get("payload", {}).get("uptime_seconds"),
            "packet_id": raw_packet_dict.get("id"),
            "rx_time": rx_time_val, 
            "rx_timestamp_iso": rx_timestamp_iso,
            "hop_limit": raw_packet_dict.get("hopLimit"), 
            "priority": raw_packet_dict.get("priority"),
            "from_id_str": raw_packet_dict.get("fromId") or raw_packet_dict.get("sender"), 
            "to_id_str": raw_packet_dict.get("toId"),
            "channel_id": raw_packet_dict.get("channelId"),
            "channel": raw_packet_dict.get("channel"), 
            "gateway_id": raw_packet_dict.get("gatewayId"),
            "hop_start": raw_packet_dict.get("hopStart") or raw_packet_dict.get("hop_start"),
            "hops_away": raw_packet_dict.get("hops_away"),
            "rssi": raw_packet_dict.get("rxRssi") or raw_packet_dict.get("rssi"),
            "snr": raw_packet_dict.get("rxSnr"), 
            ...
            "altitude": float(position_part.get("altitude")) if position_part.get("altitude") is not None else 
                    (float(raw_packet_dict.get("payload", {}).get("altitude")) if raw_packet_dict.get("payload", {}).get("altitude") is not None else None),
            ...
            "sats_in_view": position_part.get("satsInView"),
            "position_time": position_part.get("time"), 
            "ground_speed": position_part.get("groundSpeed"),
            "ground_track": position_part.get("groundTrack"),
            "user_hardware_model_id": user_part.get("hwModel") or raw_packet_dict.get("payload", {}).get("hardware"),
            "user_long_name": user_part.get("longName") or raw_packet_dict.get("payload", {}).get("longname"),
            "user_role": user_part.get("role") or raw_packet_dict.get("payload", {}).get("role"),
            "user_short_name": user_part.get("shortName") or raw_packet_dict.get("payload", {}).get("shortname"),
        ...
        if decoded_part.get("portnum") == "TRACEROUTE_APP":
            ...
            parsed_data["snr_towards"] = [float(snr) for snr in decoded_part.get("snrTowards", [])]
            parsed_data["route_back"] = decoded_part.get("routeBack", [])
            parsed_data["snr_back"] = [float(snr) for snr in decoded_part.get("snrBack", [])]
        return parsed_data
    except Exception as e:
...
def parse_meshdash_nodeinfo_packet(raw_packet_dict):
    ...
    try:
        ...
        parsed_data = {
            ...
            "channel_utilization": raw_packet_dict.get("channelUtilization", device_metrics.get("channelUtilization")) or raw_packet_dict.get("payload", {}).get("channel_utilization"),
            "air_util_tx": raw_packet_dict.get("airUtilTx", device_metrics.get("airUtilTx")) or raw_packet_dict.get("payload", {}).get("air_util_tx"),
            "uptime_seconds": device_metrics.get("uptimeSeconds", device_metrics.get("upTimeSeconds")) or raw_packet_dict.get("payload", {}).get("uptime_seconds"),
...
def create_meshdash_event(parsed_packet_data):
    ...
    if app_type_enum == PacketAppType.TRACEROUTE_APP:
        ...
        meshdash_event["snr_towards"] = [float(snr) for snr in parsed_packet_data.get("snr_towards", [])]
        meshdash_event["route_back"] = parsed_packet_data.get("route_back", [])
        meshdash_event["snr_back"] = [float(snr) for snr in parsed_packet_data.get("snr_back", [])]
